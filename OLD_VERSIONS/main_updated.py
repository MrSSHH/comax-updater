...
def save_config(data):
    """
    Save the current positions and version to a JSON file.
    """
...
def get_cursor_position(message):
    ...
    # Define a callback for mouse clicks
    def on_click(x, y, button, pressed):
        if pressed and button == mouse.Button.left:
            # Check if Ctrl is pressed during the click
            if ctrl_listener.ctrl_pressed:
                ...
                return False  # Stop listener after the click

    # Define a callback for keyboard events
    def on_press(key):
    ...
    with mouse.Listener(on_click=on_click) as listener:
        with keyboard.Listener(on_press=on_press, on_release=on_release) as ctrl_listener:
            ctrl_listener.ctrl_pressed = False
            listener.join()  # Wait until the mouse listener stops
...
def check_screen_for_image(image_path, retries=3, resize_factor=0.9, timeout=3):
    """
    Checks for an image on the screen and logs confidence levels during matching.
    """
    for _ in range(retries):
        ...
        if max_val > 0.5:  # Confidence threshold
            ...
            return max_loc  # Image found
        sleep(timeout)
        # Optionally resize template for the next attempt
        template = cv2.resize(template,
                              (int(template.shape[1] * resize_factor), int(template.shape[0] * resize_factor)))
...
def show_alert(title, message):
    """
    Show a Windows dialog alert with a given title and message.
    """
    ctypes.windll.user32.MessageBoxW(0, message, title, 0x40 | 0x1)  # 0x40 for information icon, 0x1 for OK button


def play_sound(file_path):
    """
    Play a sound from the provided file path, if sound is not disabled.
    """
...
if config:
    ...
    if use_previous:
        ...
        # Get cursor positions from user using Ctrl + Left Click
...
input("Once ready, press OK to start the update process. You will have 5 seconds to open the update window.")
